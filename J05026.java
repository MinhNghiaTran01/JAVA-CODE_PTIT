...
public class J05026 {
	public static void main(String[] args) {
		...
		while(n-->0)
		{
			String ten_bo_mon = sc.nextLine().toUpperCase();
			String s[] = ten_bo_mon.split("\\s+");
			String ma_bo_mon = "";
			for(int i=0;i<s.length;i++) ma_bo_mon+=s[i].charAt(0);
			System.out.println("DANH SACH GIANG VIEN BO MON " + ma_bo_mon + ":");
			for(Giang_Vien TMP: danh_sach_giang_vien)
			{
				if(TMP.ma_bo_mon.equals(ma_bo_mon)) System.out.println(TMP);
			}
		}
	}
}
